...
def check_llama_cpp():
    ...
    # Check if convert.py exists (from llama.cpp)
    llama_cpp_dirs = [
        Path.home() / "llama.cpp",
        Path("/opt/llama.cpp"),
...
def convert_to_gguf_fp16(model_path: str, output_dir: str, llama_cpp_path: Path):
    """
...
def quantize_gguf(fp16_file: Path, output_dir: str, quant_type: str, llama_cpp_path: Path):
    """
...
def export_to_gguf(model_path: str, output_dir: str, quantize: bool = True, quant_types: list = None):
    ...
    fp16_file = convert_to_gguf_fp16(model_path, output_dir, llama_cpp_path)
    ...
    if quantize:
        ...
        for quant_type in quant_types:
            quant_file = quantize_gguf(fp16_file, output_dir, quant_type, llama_cpp_path)
            if quant_file:
